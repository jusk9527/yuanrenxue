...
exec_bm = os.popen('node exec_b.js {}'.format(m)).read().replace('\n', '')
data = ''
for i in range(4):
    data += os.popen('node exec_b.js {}'.format(getTime())).read().replace('\n', ',')
data += os.popen('node _$TT.js').read().replace('\n', '')
...
url = 'http://match.yuanrenxue.com/api/match/5?page=5' + '&m=' + m + '&f=' + f
print(url)
